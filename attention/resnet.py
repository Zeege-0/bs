...
def ResLayer(block, inplanes, planes, blocks, stride=1, attention_module=SimAMAttention):

    block = BasicBlock

    norm_layer = nn.BatchNorm2d
    downsample_0 = nn.Sequential(
        conv1x1(inplanes, planes, stride=stride),
        norm_layer(planes)
    )
    downsample_1 = nn.Sequential(
        conv1x1(planes, planes, stride=stride),
        norm_layer(planes)
    )

    layers = []
    layers.append(block(inplanes, planes, stride, downsample_0, norm_layer=norm_layer, attention_module=attention_module))
    for _ in range(1, blocks):
        layers.append(block(planes, planes, stride, downsample_1, norm_layer=norm_layer, attention_module=attention_module))
